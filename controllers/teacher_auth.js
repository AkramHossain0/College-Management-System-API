...
const teacherRegister = async (req, res) => {
    try {
        const { name, email, number, password, designation, department, profileImage } = req.body;

        if (!name || !email || !number || !password || !designation || !department || !profileImage) {
        ...
        const userExists = await TeacherAuth.findOne({ email });
        if (userExists) {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: 'Welcome to College Management System - Email Verification',
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #f9f9f9;">
                    <div style="text-align: center; padding-bottom: 20px;">
                        ...
                        <p style="color: #555; font-size: 16px;">Thank you for joining our institution! Please verify your email to activate your account.</p>
                    </div>
...
const verifyEmail = async (req, res) => {
    try {
        ...
        const { verificationCode: code, userData } = JSON.parse(user);
        if (verificationCode !== code) {
            return res.status(400).json({ message: "Invalid verification code" });
        }

        const newUser = new TeacherAuth(userData);
        await newUser.save();
...
const teacherLogin = async (req, res) => {
    try {
        ...
        let user;
        if (email) {
            user = await TeacherAuth.findOne({ email });
        }
        else if (name) {
            user = await TeacherAuth.findOne({ name });
        }
...
const teacherForgotPassword = async (req, res) => {
    try {
        ...
        const user = await TeacherAuth.findOne({ email });
...
const teacherResetPassword = async (req, res) => {
    try {
        ...
        if (!storedData) {
            return res.status(400).json({ message: "Reset code expired. Please try again." });
        }
        ...
        await TeacherAuth.updateOne({ email }, { password: hashedPassword });
...
export { teacherRegister, verifyEmail, teacherLogin, teacherForgotPassword, teacherResetPassword };